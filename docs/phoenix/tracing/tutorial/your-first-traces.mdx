...
<Tabs>
  <Tab title="TypeScript" icon="js">
    ```bash
    npm install ai @ai-sdk/openai @arizeai/phoenix-otel zod
    ```

    <Note>
    AI SDK v7 requires Node.js 22 or newer. `@arizeai/phoenix-otel` bundles the OpenTelemetry setup and the `@arizeai/openinference-vercel` span processors that translate AI SDK spans for Phoenix.
    </Note>
  </Tab>
...
<Tabs>
  <Tab title="TypeScript" icon="js">
    Once `register()` has run, AI SDK calls are traced automatically — `register()` registers an AI SDK (v7+) telemetry integration for you, so no per-call configuration is needed:
