...
| `aiSdkTelemetry` | Whether to auto-register a Vercel AI SDK (v7+) telemetry integration when the `ai` package is installed. Defaults to `true`. |
...
- registration is skipped when your application already registered its own AI SDK telemetry integration
