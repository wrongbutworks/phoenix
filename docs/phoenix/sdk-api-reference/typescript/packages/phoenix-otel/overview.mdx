...
| `aiSdkTelemetry` | Whether to auto-register a Vercel AI SDK (v7+) telemetry integration when the `ai` package is installed. Defaults to the value of `global`, since AI SDK spans route through the global tracer provider. |
...
When the `ai` package (v7 or later) is installed, `register()` automatically registers an AI SDK telemetry integration, so calls like `generateText` and `streamText` emit spans without per-call configuration. The bundled `@arizeai/openinference-vercel` span processors translate AI SDK spans to OpenInference before export.

```ts
import { register } from "@arizeai/phoenix-otel";

register({ projectName: "my-ai-app" });

// Elsewhere in your app — traced automatically:
// const result = await generateText({ model, prompt });
```

- registration is skipped when your application already registered its own `OpenTelemetry` AI SDK integration
- the default follows the `global` option: non-global `register()` calls don't touch AI SDK telemetry unless you pass `aiSdkTelemetry: true`
- pass `aiSdkTelemetry: false` to opt out
