...
class _FilterBindings:
    ...
    upper case (``span_kind``). ``reject_unbound_names`` makes an unbound bare name a
    did-you-mean error instead of an ``attributes`` lookup. ``quantifiers`` is the set of call
    names allowed beyond the cast functions; both grains leave it empty.
    """
...
SPAN_BINDINGS = _FilterBindings(
    ...
    uppercase_names=frozenset({"span_kind"}),
